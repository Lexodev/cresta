...
; V1.5 Novembre 2015 (révision Mai 2025)
;*******************************************************************************

; Mode debug
MODE_DEBUG          = 0                 ; 0 pour désactiver, 1 pour activer
...
WaitVBL:
  ...
  IFNE    MODE_DEBUG
  move.w  #0,FrameRateCount             ; On reset le compteur de frame
  ENDC
  ...
  IFNE    MODE_DEBUG
  lea     CLFieldFrameRate,a0
  lea     FrameRateColor,a1
  move.w  FrameRateCount,d0
  add.w   d0,d0
  move.w  (a1,d0.w),2(a0)               ; La couleur correspond au framerate
  ENDC
...
MoveGlenzObject:
  ...
  andi.w  #$1ff,d0                      ; On wrap sur 512 positions
...
DrawLine:
  movem.l d0-d5/a2,-(a7)
  move.l  #DummyBlit,d5                 ; Dummy buffer pour premier point de la ligne
  cmp.w   d1,d3                         ; Y1 > Y2
  bge.s   Y1InfY2                       ; Non
...
Y1InfY2:
  ...
  move.w  d1,d4                         ; d4 = Y1
  add.w   d1,d1
  adda.w  OffsetLine(pc,d1.w),a2        ; a2 = Adresse ligne
  moveq   #0,d1
...
DYInfDX:                                ; d2 = PetitDelta, d3 = GrandDelta
  ...
  move.l  d5,BLTDPT(a6)                 ; Adresse de départ uniquement pour le premier point
  move.w  d3,BLTSIZE(a6)                ; ((GrandDelta+1) * 64) + 2
  movem.l (a7)+,d0-d5/a2
  rts

Octants:
  dc.b    2+01,2+01+$40,2+17,2+17+$40
  dc.b    2+09,2+09+$40,2+21,2+21+$40

OffsetLine:
offset    set 0
  REPT    256
  dc.w    offset
offset    set offset+GLENZPF_LINE
  ENDR
...
VBL:
  ...
  IFNE    MODE_DEBUG
  addi.w  #1,FrameRateCount             ; Incrémente le compteur de frame
  ENDC
...
FlagVBL:
  ...
  IFNE    MODE_DEBUG
FrameRateCount:                         ; Compteur pour déterminer le frame rate
  dc.w    0
FrameRateColor:                         ; Les couleurs pour le framerate
  dc.w    $00F0,$0FF0,$0F80,$0F00,$0F00,$0F00
  ENDC
...
ScrollSwingTable:
  INCLUDE 'include/scrollwave.s'

LogoSwingPtr:
  dc.l    LogoSwingTable

LogoSwingTable:
  INCLUDE 'include/logowave.s'

LogoPicture:
  INCBIN  'data/logo.data'

FontePicture:
  INCBIN  'data/font.data'
...
DummyBlit:
  ds.w    1                             ; Pour tricks du premier point de la ligne
...
StarSprite:
  ...
  INCLUDE 'include/ground_sprite.s'
...
CLLogoPalette:
  INCBIN  'data/logo.pal'               ; Palette du logo
...
starpos   SET starpos+GROUND_LEVELHT
  ...
  IFNE    MODE_DEBUG
  CWAIT   $0007,SCREEN_STARTY+SCREEN_HEIGHT+4
CLFieldFrameRate:
  CMOVE   $00F0,COLOR00
  ENDC
...
;*******************************************************************************
  ...
  INCLUDE "include/toolbox.s"
  INCLUDE "include/modplayer.s"
